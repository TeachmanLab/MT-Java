...
import edu.virginia.psyc.hobby.persistence.OA;
import edu.virginia.psyc.hobby.persistence.OARepository;
import edu.virginia.psyc.hobby.service.HobbyEmailService;
...
public class OAController extends QuestionController {
    ...
    @Autowired
    private HobbyEmailService emailService;
    ...
    RedirectView handleOA(@ModelAttribute("OA") OA oa,
        ...
        if(previous.size() > 0) {
            ...
            //Participant participant = participantService.get(principal);
            HobbyStudy study = (HobbyStudy)participant.getStudy();
